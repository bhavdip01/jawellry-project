...
const getShippingpolicy = async (req, res) => {
    try {
        const  { id,tital,page,limit} = req.query
        ...
        if(id){
            ...
            shippingPolicyData = await shippingPolicy.findOne({ _id: id })

        }
        else if(tital){

            shippingPolicyData = await shippingPolicy.find({ tital: tital}).skip(skip).limit(limit)
